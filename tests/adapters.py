...
def run_multihead_self_attention(
    ...
    """
    attn = CausalMultiHeadSelfAttention(d_model, num_heads, rope=None)
...
def run_transformer_lm(
    ...
    # Our module names the token embedding `token_embedding` (singular);
    # the reference state dict uses `token_embeddings`.
    remapped = {
        (k.replace("token_embeddings.", "token_embedding.", 1) if k.startswith("token_embeddings.") else k): v
...
def run_train_bpe(
    ...
    **kwargs,
    ...
    return train_bpe(input_path, vocab_size, special_tokens, num_processes=10)
